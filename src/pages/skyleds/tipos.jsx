...
export function TiposSkylesd() {
  ...
  // ENTRADAS MULTIPLES
  const [entradasMultiples, setEntradasMultiples] = useState([{ producto_id: '', cantidad: 1 }]);
  const [entradaGlobal, setEntradaGlobal] = useState({ novedad: '' });
  ...
  useEffect(() => {
    ...
    // Cargar salidas y entradas al inicio
    cargarSalidas();
    cargarEntradas();
  }, []);
  ...
  const agregarProductoEntrada = () => {
    setEntradasMultiples([...entradasMultiples, { producto_id: '', cantidad: 1 }]);
  };

  const actualizarEntrada = (index, campo, valor) => {
    const nuevas = [...entradasMultiples];
    if (campo === 'cantidad') {
      const num = parseInt(valor, 10);
      nuevas[index][campo] = isNaN(num) ? '' : num;
    } else {
    ...
    setEntradasMultiples(nuevas);
  };

  const registrarEntradasMultiples = () => {
    const productosValidos = entradasMultiples
      .filter(e => e.producto_id && !isNaN(parseInt(e.cantidad)) && parseInt(e.cantidad) > 0)
      .map(e => ({
        producto_id: parseInt(e.producto_id, 10),
        cantidad: parseInt(e.cantidad, 10)
      }));

    const payload = {
      novedad: entradaGlobal.novedad,
      productos: productosValidos
    };

    axios.post('http://localhost:3001/entradas', payload)
      .then(() => {
        alert('Entrada registrada');
        cargarEntradas();
        setEntradasMultiples([{ producto_id: '', cantidad: 1 }]);
        setEntradaGlobal({ novedad: '' });
      })
      .catch(err => {
        console.error(err);
  ...
  const verDetalleEntrada = (entradaId) => {
    setEntradaSeleccionada(entradaId);
    axios.get(`http://localhost:3001/entradas/${entradaId}/detalle`)
      .then(res => setDetalleEntrada(res.data));
  };
  ...
  return (
    <>
      ...
      <div className='items-center mx-auto max-w-4xl px-4 w-full'>
        ...
        {/* FIN NUEVO DISEÑO REGISTRAR ENTRADA */}
        ...
        <div className='bg-black h-1 w-full mb-20'/>
        {/* NUEVO DISEÑO REGISTRAR ENTRADA */}
        <div className="bg-white rounded-xl shadow-lg p-6 mb-8 mx-auto">
          <h3 className="text-2xl font-semibold mb-4 flex items-center gap-2">
            Registrar entrada
          </h3>
          <div className="space-y-4">
            {entradasMultiples.map((e, i) => (
              <div key={i} className="flex flex-col md:flex-row items-center gap-3 bg-default-100 rounded-lg p-4 shadow-sm">
                <div className="flex-1">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Producto</label>
                  <select
                    className="w-full border border-gray-300 rounded px-2 py-1"
                    value={e.producto_id}
                    onChange={ev => actualizarEntrada(i, 'producto_id', ev.target.value)}
                  >
                    <option value="">Seleccionar producto</option>
                    {Object.values(productosPorCategoria).flat().map(p => (
                      <option key={p.id} value={p.id}>
                ...
                <div>
                  ...
                  <input
                    ...
                    min="1"
                    onChange={ev => actualizarEntrada(i, 'cantidad', ev.target.value)}
                    placeholder="Cantidad"
                  />
                </div>
                {entradasMultiples.length > 1 && (
                  <button
                    className="ml-2 text-red-500 hover:text-red-700"
                    title="Eliminar producto"
                    onClick={() => setEntradasMultiples(entradasMultiples.filter((_, idx) => idx !== i))}
                  >
                    <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 6l8 8M6 14L14 6" /></svg>
                  </button>
                )}
          ...
          <div className="mt-6">
            ...
            <input
              className="w-full border border-gray-300 rounded px-2 py-1"
              placeholder="Novedad"
              value={entradaGlobal.novedad}
              onChange={e => setEntradaGlobal({ ...entradaGlobal, novedad: e.target.value })}
            />
          </div>
          <button
            className="mt-6 w-full bg-default-100 text-gray-500 font-semibold py-2 rounded-lg shadow hover:bg-default-300 transition"
            onClick={registrarEntradasMultiples}
          >
            Registrar entrada
          </button>
        </div>
        ...
        <div className='max-w-7xl items-center mx-auto mb-20'>
          <h3>Entradas registradas</h3>
          <button onClick={cargarEntradas}>Recargar entradas</button>
          <Table
            aria-label="Entradas registradas"
          >
            <TableHeader>
              <TableColumn>ID</TableColumn>
              <TableColumn>Fecha</TableColumn>
            ...
            <TableBody>
              {entradas.length === 0 ? (
                <TableRow key="1">
                  <TableCell colSpan={3}>No hay entradas registradas.</TableCell>
                </TableRow>
              ) : (
                entradas.map(e => (
                  <TableRow className='hover:bg-black/5' key={e.id} onClick={() => verDetalleEntrada(e.id)}>
          ...
          {/* Detalle de productos de la entrada seleccionada en un modal */}
          {entradaSeleccionada && (
            <div
              style={{
                position: 'fixed',
              ...
              onClick={() => { setEntradaSeleccionada(null); setDetalleEntrada([]); }}
            >
              <div
                style={{
                  background: 'white',
                ...
                onClick={e => e.stopPropagation()}
              >
                <h4>Productos de la entrada #{entradaSeleccionada}</h4>
                <button
                  style={{
                    position: 'absolute',
                    top: 8,
                ...
                <table border="1" cellPadding={5} style={{ width: '100%', marginTop: 16 }}>
                  ...
                  <tbody>
                    {detalleEntrada.length === 0 ? (
                      <tr>
                        <td colSpan={3}>No hay productos para esta entrada.</td>
                      </tr>
                    ) : (
                      detalleEntrada.map((d, idx) => (
                        <tr key={idx}>
                          <td>{d.producto}</td>
                          <td>{d.referencia}</td>
                          <td>{d.cantidad}</td>
