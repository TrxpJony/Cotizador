...
const Colosalxx = () => {
  ...
  const guiaSimplecolPrice = dbPrices.guiaSimplecol ? (dbPrices.guiaSimplecol * (totalWidth / 1000)) : 0;
  const jambacolPrice = dbPrices.jambacol ? (dbPrices.jambacol * (doubleHeight / 1000)) : 0;
  const horizontalSuperiorcolPrice = dbPrices.horizontalSuperiorcol ? (dbPrices.horizontalSuperiorcol * (doubleHalfWidth / 1000)) : 0;
  const horizontalInferiorMovilcolPrice = dbPrices.horizontalInferiorMovilcol ? (dbPrices.horizontalInferiorMovilcol * (doubleHalfWidth / 1000)) : 0;
  const traslapecolPrice = dbPrices.traslapecol ? (dbPrices.traslapecol * (doubleHeight / 1000)) : 0;
  const enganchecolPrice = dbPrices.enganchecol ? (dbPrices.enganchecol * (doubleHeight / 1000)) : 0;
  ...
  const [componentTotals, setComponentTotals] = useState({
    ...
    guiaSimple: { totalSize: 0, totalPrice: 0 },
  ...
  const handleAddDoor = () => {
    ...
    setComponentTotals((prevTotals) => ({
      ...
      guiaSimple: {
        totalSize: prevTotals.guiaSimple.totalSize + parseFloat(totalWidth),
        totalPrice: prevTotals.guiaSimple.totalPrice + guiaSimplecolPrice,
      },
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(10);
    doc.text('Detalle de la cotización Colosal PC 2.6 XX', 70, 34);
    ...
    addTableRow(doc, 60, 'Guia Simple:', `${componentTotals.guiaSimple.totalSize} mm`, `${componentTotals.guiaSimple.totalPrice.toFixed(2)}`);
    ...
    addSection(doc, 'Accesorios', 105);
    addTableRow(doc, 110, 'Kit de Cierre:', `${accessoryTotals.kitCierre.cantidad}`, `${accessoryTotals.kitCierre.totalPrice.toFixed(2)}`);
    addTableRow(doc, 115, 'Kit de Cierre con Llave:', `${accessoryTotals.kitCierreConLlave.cantidad}`, `${accessoryTotals.kitCierreConLlave.totalPrice.toFixed(2)}`);
    ...
    addTableRow(doc, 125, 'Rodamiento Simple en Agujas:', `${accessoryTotals.rodamientoSimple.cantidad}`, `${accessoryTotals.rodamientoSimple.totalPrice.toFixed(2)}`);
    addTableRow(doc, 130, 'Rodamiento Doble en Agujas:', `${accessoryTotals.rodamientoDoble.cantidad}`, `${accessoryTotals.rodamientoDoble.totalPrice.toFixed(2)}`);
