...
const handleGlassChange = (e) => {
  ...
  const totalPrice =
    ...
    const generatePDF = () => {
      ...
      doc.text(`${doubleHalfWidth} mm (2)`, 120, 85);
      ...
      doc.text(`${empaquecolPrice.toFixed(2)}`, 170, 172.5);
  ...
  return (
    <div className="door-container">
      <div className="door-frame">
        ...
        <button onClick={handleAddDoor} className="add-door-btn">
        Agregar Puerta
      </button>

      <div className="doors-summary">
        <h2>Resumen de Puertas</h2>
        <ul>
          {puertas.map((puerta, index) => (
            <li key={index}>
              Puerta {index + 1}: {puerta.dimensions.height} mm x {puerta.dimensions.width} mm - ${puerta.price.toFixed(2)}
            </li>
          ))}
        </ul>
        <p>
        ...
        <p>
          <strong>Precio Total:</strong> ${totalSum.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </p>
      </div>
      <br />
         {/* Botón para regresar */}
