...
export function Zinupage() {
  const navigate = useNavigate(); //Inicializa el hook para la navegación
  const [list, setList] = useState(); // Datos de la API
  const [filteredList, setFilteredList] = useState([]); // Solo mantenemos el estado para la lista filteredData
    ...
    fetch(baseUrl)
      ...
      .then((data) => {
        if (data && Array.isArray(data)) {
          ...
          const categoriaData = data.filter(item => item.categoria?.toLowerCase() === 'zinu');
